...
class BotVK:
    ...
    def find_age_low(self, user_id):
        ...
        try:
            ...
            self.send_msg(user_id, 'Неверный токен')
    ...
    def find_age_high(self, user_id):
        ...
        try:
            ...
            if len(bday_list) == 3:
                ...
                self.send_msg(user_id, 'Укажите возраст для поиска до ')
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                        msg = event.text
                        return msg
    ...
    def find_gender(self, user_id):
        ...
        try:
            ...
            if gender == 1:
                ...
                self.send_msg(user_id, 'Укажите пол для поиска("м" или "ж")')
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                        msg = event.text
                        if msg.lowwer() == 'м':
    ...
    def find_city(self, user_id):
        ...
        try:
            if 'city' in user_data['response'][0]:
                ...
                self.send_msg(user_id, 'Ввидите город проживания для поиска')
                for event in self.longpoll.listen():
